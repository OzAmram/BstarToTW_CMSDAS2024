...
samples = {     
        ...
        "QCDHT1500"     : "QCD",
        "QCDHT2000"     : "QCD",
        "QCD"           : "QCD",                # used to concatenate the 4 QCD processes
        "singletop"     : "single top (tW)"     # used to concatenate the two singletop processes
}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rescaleQCD',
                        action='store_true',
                        help='If flag passed QCD cross section is ignored and instead QCD is scaled to the same yield as ttbar (useful for visualization)')
    ...
    for varname in ExistingVarnames:
        ...
        # QCD has a *LOT* more entries than the other backgrounds, so can choose to normalize to ttbar before plotting
        # first, just check if these histos exist for the year, if not then skip
        if (args.rescaleQCD) and ('QCD' in bkg_hists) and ('ttbar' in bkg_hists):
