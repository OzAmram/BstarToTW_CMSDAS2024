...
def nminus1(setname, year):
    ...
    #wtag_str = "0*Wtag(Dijet_tau2[0]/Dijet_tau1[0],0,{0},Dijet_msoftdrop[0],65,105) + 1*Wtag(Dijet_tau2[1]/Dijet_tau1[1],0,{0},Dijet_msoftdrop[1],65,105)".format(cuts['tau21'])
    wtag_str = "1*(Dijet_msoftdrop[1] < Dijet_msoftdrop[0])"

    # Create a TIMBER VarGroup to store information about the jets
    jets = VarGroup('jets')
    jets.Add('w_index',    wtag_str)
    ...
    plotting_vars.Add("tau32", "Dijet_tau3[top_index]/(Dijet_tau2[top_index] + eps)")   # Assume the lead is the top...
    plotting_vars.Add("tau21", "Dijet_tau2[w_index]/(Dijet_tau1[w_index] + eps)")   # and the sublead is the W

    N_cuts = CutGroup('Ncuts') # N cuts to make on the VarGroup above
    N_cuts.Add("deltaY_cut",      "deltaY < %.3f" % abs(cuts['deltaY'][0]))
    N_cuts.Add("mtop_cut",        "(mtop > 105.) && (mtop < 220.)")
    N_cuts.Add("mW_cut",          "(mW > 65.) && (mW < 105.)")
    N_cuts.Add("tau32_cut",       "(tau32 > 0.0) && (tau32 < %.3f)"%(cuts['tau32'])) # These cut values are taken from the config! 
    N_cuts.Add("tau21_cut",       "(tau21 > 0.0)&&(tau21 < %.3f)"%(cuts['tau21']))
